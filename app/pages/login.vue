	...
	>
		<Card class="relative w-full max-w-md p-8 space-y-6 rounded-2xl shadow-lg">
			...
			<CardHeader class="text-center pt-12">
				<CardTitle class="text-3xl font-bold">Sign in to continue</CardTitle>
				<CardDescription class="text-muted-foreground"
					>Please sign in to start browsing!</CardDescription
				>
			</CardHeader>
			<CardContent>
...
const handleLogin = async () => {
	...
	loading.value = true;
	const { ok, error } = await sessionStore.signInWithPassword(
		email.value,
		password.value
	);
	loading.value = false;
